...
def is_eye_open_hough(eye_roi_bgr):
    ...
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, dp=1, minDist=10, param1=60, param2=20, minRadius=3,
                               maxRadius=30)
...
def main():
    ...
    while True:
        ...
        try:
            faces_front, _, levelWeights_f = face_cascade_frontal.detectMultiScale3(gray, scaleFactor=1.1,
            ...
            faces_front = face_cascade_frontal.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=3,
                                                                minSize=(100, 100), maxSize=(500, 500))

        try:
            faces_prof, _, levelWeights_p = face_cascade_profile.detectMultiScale3(gray, scaleFactor=1.1,
                                                                                   minNeighbors=3, minSize=(100, 100),
                                                                                   maxSize=(500, 500),
                                                                                   outputRejectLevels=True)
            faces_prof = [bbox for bbox, lw in zip(faces_prof, levelWeights_p) if lw > level_weight_threshold]
        except:
            faces_prof = face_cascade_profile.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=3,
                                                               minSize=(100, 100), maxSize=(500, 500))
        ...
        for (x, y, w, h) in faces:
            ...
            for (mx, my, mw, mh) in mouths:
                ...
                break
            cv2.putText(frame, f"Eye: {predicted_eye_state}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255),
                        2)
